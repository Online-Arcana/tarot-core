...
test("reader-dialogue gender and direct-address faults use one field-only Spanish correction", async () => {
  ...
  const correctedCard = "En el futuro, el Cinco de Espadas advierte de conflicto. La clave será decidir qué no quieres sacrificar para tener razón o imponerte.";
  const correctedSynthesis = "Cuando miras las tres cartas juntas, aparece una secuencia clara: una decepción del pasado sigue pesando, el presente mezcla deseo con fantasía y el futuro pide atención al conflicto.";
  const calls = [];
  const fetch = async (_url, init) => {
    ...
    return response(calls.length === 1
      ? primary
      : { cardText_2: correctedCard, synthesis: correctedSynthesis });
  };
  ...
  assert.equal(result.out.cardText[0], primary.cardText[0]);
  assert.equal(result.out.cardText[1], primary.cardText[1]);
  assert.equal(result.out.cardText[2], correctedCard);
  assert.equal(result.out.synthesis, correctedSynthesis);
  assert.equal(result.out.reading, primary.reading);
  assert.equal(result.out.closing, primary.closing);
  assert.equal(auditModelOut(req, result.out).valid, true);
  assert.ok(result.auditErrors.some(value => value.includes("narrow_spanish_narrator_correction:read.cardText[2],read.synthesis")));

  const patch = calls[1];
  assert.equal(patch.text.format.name, "arcana_spanish_prose_patch");
  assert.deepEqual(Object.keys(patch.text.format.schema.properties), ["cardText_2", "synthesis"]);
  assert.deepEqual(patch.text.format.schema.required, ["cardText_2", "synthesis"]);
