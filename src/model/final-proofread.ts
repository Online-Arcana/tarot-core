...
function scopedFields(
  ...
  const all = proofreadFields(req, out);
  if (paths === undefined) return all;
  const unique = [...new Set(paths)];
  for (const path of unique) {
    if (!(path in all)) throw new Error(`Final proofread path ${path} is not editable for ${req.task}`);
  }
...
export function finalProofreadShape(
  req: ApiReq,
  out: ApiOut,
  allowedPaths?: readonly string[],
) {
  const fields = scopedFields(req, out, allowedPaths);
...
export function finalProofreadPrompt(
  ...
  options: ProofreadPromptOptions = {},
): string {
  const fields = scopedFields(req, out, options.paths);
