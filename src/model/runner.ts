...
export async function runModelSession(
  ...
  try {
    if (narrow !== null && primary !== undefined) {
      const patch = await ai.run(
        narrowCorrectionShape(narrow.paths),
        [{ role: "system", content: narrowCorrectionPrompt(req, primary, narrow.paths) }],
        sendOpts(cfg, escalationModel),
